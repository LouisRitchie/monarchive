...
function init() {
  ...
  document.querySelector("#submit-button").addEventListener('click', () => {
    ...
    EDIT_RECORD_CHANNEL.push("submit", params).receive("ok", () => {
      window.location = `/record/${window.location.pathname.split('/')[2]}`
    })
  })
}
...
(function () {
  ...
  const edit_record_channel_id = `edit_record:${id}`
  const image_uploader_channel_id = `image_uploader:${id}`
  socket.connect()
  EDIT_RECORD_CHANNEL = socket.channel(edit_record_channel_id, {})
  IMAGE_UPLOADER_CHANNEL = socket.channel(image_uploader_channel_id, {})

  EDIT_RECORD_CHANNEL.join()
    .receive("ok", resp => {
      console.log(`Joined ${edit_record_channel_id} successfully`, resp)
